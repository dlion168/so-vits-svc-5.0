...
from whisper_local.inference import load_model, pred_ppg
...
if __name__ == '__main__':
    ...
    for file in tqdm.tqdm(waves, desc="svc"):
        os.system(
            f"python svc_inference.py --config {args.config} --model {args.model} --wave {wave_path}/{file} --ppg {out_path}/{file}.ppg.npy --spk {args.spk} --shift {args.shift} --tgt_median_pitch {args.tgt_median_pitch}")
        os.system(f"mv svc_out.wav {out_path}/wav/{file}")
